...
const ProjectCard = ({ title, episodes, lastedited, id }) => {
  ...
  return (
    <div
      className="w-[270px] flex items-center bg-white border border-gray-400 rounded-2xl p-4 cursor-pointer"
      onClick={() => {
        navigate(`/project/${title}/add-podcast`);
        fetchFiles(id, title);
        setProjectId(id);
        console.log(id, "this is the project id i had set to it");
      }}
    >
