...
const SideBar = () => {
  ...
  return (
    <div className='h-full flex flex-col items-center'>
        <img src={QuestLogo} alt="Quest Logo" className="w-[8rem] h-[8rem] object-contain flex-shrink mb-[-1.5rem]" />
        {sidebarItems.map((item , index) => {
            const isActive = item.to == location.pathname.split('/').pop();
            return  <SidebarButton key={index} to={`/project/${id}/${item.to}`} icon={item.icon} active={isActive}>{item.label}</SidebarButton>
        })}
        ...
        <div className='mt-auto mb-10 w-full flex flex-col justify-center items-center'>
            <SidebarButton to={`/project/${id}/help`} icon={Settings} active={location.pathname.split('/').pop() == 'help'} >Help</SidebarButton>
            <div className='w-[80%] h-[1px] bg-slate-300 mt-2'></div>

            <div className='flex items-center justify-start ml-12 mt-4 w-full cursor-pointer' onClick={() => navigate(`/project/${id}/settings`)}>
                <img src={ProfilePic} alt="icon" className="w-12 h-12 object-contain" />
                <div className='flex flex-col ml-4'>
                    <span className="text-lg font-medium">{userName ? userName : "John Doe"}</span>
                    <span className="text-sm text-gray-500">{email ? email : "john.doe@quest.com"}</span>
                </div>
            </div>
        </div>
    </div>
