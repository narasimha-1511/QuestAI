        ...
        <div className="flex flex-col w-full h-full px-16 p-6">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
              {isSaved ? (
                <button
                  className="bg-black text-white px-4 py-2 rounded-md w-[100px]"
                ...
                <>
                  <button
                    className="bg-transparent text-[#E01919] border border-[#E01919] px-4 py-2 font-bold rounded-md w-[100px]"
                    onClick={() => {
                      setEditTranscript(false);
                      setTransscriptContent("");
                    }}
                  >
                    Discard
                  </button>
          ...
          <div className="px-8 mt-8 bg-white w-full h-[65vh] rounded-lg shadow-lg flex justify-center items-center">
            <textarea
              readOnly={isSaved}
              className={`w-full h-[60vh] py-2 resize-none outline-none`}
              value={TransscriptContent}
              onChange={(e) => setTransscriptContent(e.target.value)}
            ></textarea>
          </div>
        </div>
      ) : (
